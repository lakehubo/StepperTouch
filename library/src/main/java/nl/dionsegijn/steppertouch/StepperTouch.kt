...
class StepperTouch : ConstraintLayout {
    ...
    private fun handleAttrs(attrs: AttributeSet) {
        ...
        if(isVertical){
            LayoutInflater.from(context).inflate(R.layout.stepper_touch_vertical, this, true)
        }else {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (allowDragging) {
                    if(isVertical) {
                        ...
                        var movex = event.y - startY
                        viewCounter.translationY = event.y - startY
                    } else {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if(isVertical){
                    if (viewCounter.translationY != 0f) {
                        val animY = SpringAnimation(viewCounter, SpringAnimation.TRANSLATION_Y, 0f)
                        animY.spring.stiffness = stiffness
                        animY.spring.dampingRatio = damping
                        animY.start()
                    }
                } else {
